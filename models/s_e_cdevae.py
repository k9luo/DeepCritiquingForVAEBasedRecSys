    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('vae'):
            ...
            with tf.variable_scope('latent'):
                ...
                ##
                std_abs = tf.abs(logstd)
                std_mean_abs = tf.reduce_mean(std_abs)
    ...
    def train_rating(self, rating_matrix, keyphrase_matrix, corruption, epoch=100, batches=None, **unused):
        ...
        for i in pbar:
            ...
            for step in range(len(batches)):
                ...
                training, loss, summary = self.sess.run([self._train_rating, self._loss_rating, self.merged], feed_dict=feed_dict)
                pbar.set_description("loss:{}".format(loss))
                self.writer.add_summary(summary,c)
...
def s_e_cde_vae(matrix_train, matrix_train_keyphrase, embeded_matrix=np.empty((0)),
              epoch_rating=100, epoch_exp = 100, lamb_l2=80.0, lamb_keyphrase=1.0, lamb_latent=5.0, lamb_rating=1.0,
    ...
    model.train_rating(matrix_input, matrix_input_keyphrase, corruption, epoch_rating)
    model.train_keyphrase(matrix_input, matrix_input_keyphrase, corruption, epoch_exp)
